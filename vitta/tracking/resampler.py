...
class ResampledRecord:
    ...
    # ── Metre-based fields (populated when pixels_per_metre is set) ──
    speed_m_per_sec: Optional[float] = None
    speed_kmph: Optional[float] = None
    cumulative_distance_m: Optional[float] = None

    # ── Direction (populated after resampling from trajectory vector) ──
    ...
    def to_dict(self) -> dict:
        ...
        if self.speed_m_per_sec is not None:
            d["speed_m_per_sec"] = round(self.speed_m_per_sec, 4)
        if self.speed_kmph is not None:
            d["speed_kmph"] = round(self.speed_kmph, 2)
        if self.cumulative_distance_m is not None:
            d["cumulative_distance_m"] = round(self.cumulative_distance_m, 4)
...
class TrackResampler:
    ...
    def __init__(self, interval_sec: float = 1.0, pixels_per_metre: Optional[float] = None):
    ...
    def resample(self) -> List[ResampledRecord]:
        ...
        for track_id in sorted(self._tracks.keys()):
            ...
            for target_sec in sample_times:
                ...
                # Compute metre-based values if calibration is available
                speed_m = None
                speed_km = None
                dist_m = None
                if self.pixels_per_metre is not None and self.pixels_per_metre > 0:
                    speed_m = speed / self.pixels_per_metre
                    speed_km = speed_m * 3.6
                    dist_m = cumulative_dist / self.pixels_per_metre
        ...
        self._assign_directions(all_records)
    ...
    @staticmethod
    def _assign_directions(records: List[ResampledRecord]) -> None:
        """
        Compute dominant direction for each track from first→last centroid
        displacement and assign to all records of that track.

        Directions use compass labels relative to image coordinates:
        - Northbound  = moving UP    (negative dy)
        - Southbound  = moving DOWN  (positive dy)
        - Eastbound   = moving RIGHT (positive dx)
        - Westbound   = moving LEFT  (negative dx)
        """
        # Group records by track_id
        from collections import defaultdict
        by_track: Dict[int, List[ResampledRecord]] = defaultdict(list)
        for rec in records:
            by_track[rec.track_id].append(rec)

        for tid, recs in by_track.items():
            if len(recs) < 2:
                # Can't determine direction from a single point
                for r in recs:
                    r.direction = "Stationary"
                continue

            first = recs[0]
            last = recs[-1]
            dx = last.cx_px - first.cx_px
            dy = last.cy_px - first.cy_px

            # atan2 gives angle from positive-x axis
            angle = math.atan2(dy, dx) * 180.0 / math.pi  # [-180, 180]

            # Map to cardinal direction
            # Image coords: +y is down, +x is right
            if -45 <= angle < 45:
                direction = "Eastbound"
            elif 45 <= angle < 135:
