...
# Extra columns appended when spatial calibration is provided
_CSV_HEADER_METRIC = [
    "speed_m_per_sec",
    "speed_kmph",
...
class TrackCSVWriter:
    ...
    def __init__(self, output_path: str | Path, pixels_per_metre: Optional[float] = None):
        """
        ...
        Args:
            ...
            pixels_per_metre: Optional spatial calibration factor. When set,
                              three additional columns are written:
                              speed_m_per_sec, speed_kmph, cumulative_distance_m.
        """
        ...
        if self.pixels_per_metre is not None and self.pixels_per_metre > 0:
            header.extend(_CSV_HEADER_METRIC)
        ...
        logger.info(f"CSV writer opened: {self.output_path} (pixels_per_metre={self.pixels_per_metre})")
    ...
    def write_frame(
        ...
        for obj in tracked_objects:
            ...
            # Append metre-based columns when calibration is available
            if self.pixels_per_metre is not None and self.pixels_per_metre > 0:
