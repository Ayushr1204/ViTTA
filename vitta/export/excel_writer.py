...
_TIME_FIELDS_PX = ["cx_px", "cy_px", "speed_px_per_sec", "cumulative_distance_px"]
_TIME_FIELDS_METRIC = ["speed_m_per_sec", "speed_kmph", "cumulative_distance_m"]
...
_TIME_FIELD_LABELS = {
    ...
    "cumulative_distance_m": "Cumul. Distance (m)",
}
...
class ExcelExporter:
    ...
    def export(
        ...
        # Determine whether calibration data is available in the records
        has_metric = any(
            r.speed_m_per_sec is not None for r in records
        )
        ...
        self._write_trajectory_sheet_wide(ws_traj, records, has_metric)

        # ── Sheet 2: Track Summary ───────────────────────────────────
        ws_summary = wb.create_sheet("Track Summary")
        self._write_summary_sheet(ws_summary, records, has_metric)
        ...
        logger.info(
            ...
            f"({num_tracks} tracks, wide format, metric={'yes' if has_metric else 'no'})"
        )
        return output_path
    ...
    def _write_trajectory_sheet_wide(
        ...
        # Select time-field columns based on whether metric data is available
        time_fields = list(_TIME_FIELDS_PX)
        if has_metric:
            time_fields.extend(_TIME_FIELDS_METRIC)
        ...
        for _ in sorted_timestamps:
            for fld in time_fields:
                sub_header.append(_TIME_FIELD_LABELS[fld])
        ...
        for tid in sorted(tracks.keys()):
            ...
            for ts in sorted_timestamps:
                ...
                if rec is not None:
                    ...
                    if has_metric:
                        values.extend([
                            round(rec.speed_m_per_sec, 4) if rec.speed_m_per_sec is not None else None,
                            round(rec.speed_kmph, 2) if rec.speed_kmph is not None else None,
                            round(rec.cumulative_distance_m, 4) if rec.cumulative_distance_m is not None else None,
                        ])
                    row.extend(values)
                else:
                    # Track not present at this timestamp
                    row.extend([None] * num_fields)
    ...
    def _write_summary_sheet(
        ...
        headers = list(self._SUMMARY_HEADERS)
        if has_metric:
            headers.extend(self._SUMMARY_HEADERS_METRIC)
        ws.append(headers)
        ...
        for tid in sorted(tracks.keys()):
            ...
            if has_metric:
                total_dist_m = recs[-1].cumulative_distance_m if recs and recs[-1].cumulative_distance_m is not None else None
                speeds_m = [r.speed_m_per_sec for r in recs if r.speed_m_per_sec is not None]
                speeds_km = [r.speed_kmph for r in recs if r.speed_kmph is not None]
                avg_speed_m = round(sum(speeds_m) / len(speeds_m), 4) if speeds_m else None
                max_speed_m = round(max(speeds_m), 4) if speeds_m else None
                avg_speed_km = round(sum(speeds_km) / len(speeds_km), 2) if speeds_km else None
                max_speed_km = round(max(speeds_km), 2) if speeds_km else None
                row.extend([
                    round(total_dist_m, 4) if total_dist_m is not None else None,
                    avg_speed_m,
                    max_speed_m,
                    avg_speed_km,
                    max_speed_km,
