...
- **1-second interval resampling** — Converts per-frame data into clean, evenly-spaced time series
- **Speed & distance computation** — Instantaneous speed (px/s) and cumulative distance per vehicle
- **Excel export** — Multi-sheet `.xlsx` workbook with wide-format trajectory data, track summaries, and processing metadata
...
- Runs full YOLO + ByteTrack on the entire video
- Shows an annotated preview every 3rd processed frame while running
- Generates `tracks.csv` and provides one-click CSV download
- Provides a minimal dashboard-style UI with downloadable annotated video
...
| Track ID | Class | Duration (s) | Total Distance (px) | Avg Speed (px/s) | Max Speed (px/s) |
|----------|-------|-------------|---------------------|------------------|------------------|
| 1 | Car | 12 | 1543.2 | 128.6 | 195.3 |
